...
- 交互：**悬停图标 → 显示面板**（截获 pystray Win32 后端的 WM_MOUSEMOVE 作为悬停心跳；
  看门狗线程在"不悬停图标且鼠标不在面板内"超过 0.8s 后自动隐藏）；左键为显示面板的保底入口；
  右键 → 立即刷新 / 开机自启（勾选项）/ 退出。

### 6.2 弹出面板（panel.py + panel.html）

- pywebview 无边框窗口，约 360×300，置顶，`easy_drag` 关闭。
- 出现位置：屏幕工作区右下角（贴近托盘）。定位与显示用 ctypes `SetWindowPos`（`SWP_NOACTIVATE`，
  悬停显示不抢焦点）直接操作原生句柄——pywebview 的 `move()` 期望逻辑像素并自行做 DPI 缩放，
  高分屏下会双重缩放飞出屏幕。隐藏：悬停看门狗 + 失焦（JS `blur`）双保险，所有隐藏路径统一走
  `Panel.hide()` 维护可见性标志。
- 注意：暴露给 JS 的 `Api` 对象上**禁止公开属性引用 Panel/Window**——pywebview 会递归遍历
  js_api 公开属性生成 JS 桥，循环引用会卡死窗口创建（进而挂起一切 show/move 调用）。
- 内容：每个 Provider 一张卡片——
